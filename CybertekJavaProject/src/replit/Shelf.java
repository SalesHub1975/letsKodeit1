...
public class Shelf {
    ...
    public static boolean addBook(int id, String author, String title, int pages ){
        if(isIDUnique(id) == false && title.isEmpty() && pages <= 0) {
            return false;
        }else {
            Book b = new Book();
            theBooks.add(b);
    ...
    public static boolean addBook(Book book ){
        if(isIDUnique(book.getId()) == false || book.getTitle().isEmpty() || book.getPages() <= 0) {
            return false;
        }else {
            theBooks.add(book);
            return true;
    ...
    public static void removeBook(String author) {
      List<Book> list = getAllBooksForAuthor(author);
        for(Book b: list){
            theBooks.remove(b);
        }
